...
- **`@restart/grid`** — Compatibility re-export of `grid-core`. Kept while
  ...
  helpers) used to live here — they moved to `@restart/ui` on 2026-04-26
  because they don't belong in the shared papergrid submodule.

- **[`@restart/bloom-grid`](./bloom-grid/README.md)** — Search-driven grid
...
- **`BulkOrderGrid`** (in `@restart/ui`) — wholesale procurement grid. Wraps
  RichGrid; adds tier rail, summary bar, qty steppers, hover-expand with
  product detail, region-aware money formatting. Lives in `@restart/ui`
...
- **`WholesaleGlideGrid`** (in `@restart/ui`) — alternative wholesale grid
...
- `@restart/ui` (BulkOrderGrid, WholesaleGlideGrid)
